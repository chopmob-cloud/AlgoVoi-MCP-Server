...
function _parseMppWwwAuth(header: string): Record<string, unknown> {
  const result: Record<string, unknown> = { raw_header: header };
  if (!header.toLowerCase().startsWith("payment")) return result;
  const tokenM    = /token="([^"]+)"/.exec(header);
  const resourceM = /resource="([^"]+)"/.exec(header);
  if (resourceM) result.resource_id = resourceM[1];
  if (tokenM) {
    try {
      const raw    = tokenM[1];
      const padded = raw + "=".repeat((-raw.length) & 3);
      const decoded: unknown = JSON.parse(Buffer.from(padded, "base64").toString("utf-8"));
      result.challenge = decoded;
      if (decoded && typeof decoded === "object" && !Array.isArray(decoded)) {
        const d = decoded as Record<string, unknown>;
        if ("accepts"  in d) result.accepts  = d.accepts;
        if ("expires"  in d) result.expires  = d.expires;
        if ("intent"   in d) result.intent   = d.intent;
        if ("resource" in d) result.resource = d.resource;
      }
    } catch {
      result.token_raw = tokenM[1];
    }
  }
  return result;
}

export async function tryMppEndpoint(
  _client: AlgoVoiClient,
  args: TryMppEndpointInput,
): Promise<unknown> {
  ...
  try {
    ...
    if (res.status === 402) {
      ...
      return { status: 402, payment_required: true, ..._parseMppWwwAuth(wwwAuth) };
    }
    ...
    return { error: "unreachable", detail: String(err) };
  }
}
...
export const TOOL_SCHEMAS = [
  ...
  // ── MPP consumer probe ────────────────────────────────────────────────────
  {
    ...
    description:
      "Probe any MPP-protected URL. GETs (or uses the specified method) the URL and, " +
      "if the server responds with HTTP 402, parses the `WWW-Authenticate: Payment` header " +
      "to decode the base64-encoded challenge token — returning resource_id, the accepts[] " +
      "array (per-chain payment details), expires timestamp, and the full challenge object. " +
      "If the resource is accessible without payment, returns status + body preview. " +
      "Mirrors `npx agentcash try <url>` for programmatic use by AI agents. " +
      "URL must be https://.",
    inputSchema: {
      type: "object",
      properties: {
        url: {
          type:        "string",
          description: "Full https:// URL of the MPP-protected resource to probe.",
        },
        method: {
          type:        "string",
          description: "HTTP method to use (default GET). One of GET, HEAD, POST, PUT, PATCH.",
        },
      },
      required:             ["url"],
      additionalProperties: false,
