...
const EXT_NETWORKS = new Set<string>(["algorand_mainnet", "voi_mainnet", "algorand_mainnet_algo", "voi_mainnet_voi"]);
...
export function parsePrepareExtensionPayment(
  ...
  const net = requireEnum(obj, "network", ["algorand_mainnet", "voi_mainnet", "algorand_mainnet_algo", "voi_mainnet_voi"] as const)!;
  return {
    amount:   requireNumber(obj, "amount",  { gt: 0, le: 10_000_000 })!,
    currency: requireString(obj, "currency", { min: 3, max: 3 })!,
    label:    requireString(obj, "label",    { min: 1, max: 200 })!,
